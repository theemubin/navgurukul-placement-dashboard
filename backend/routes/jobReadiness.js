...
router.post('/config/seed', auth, authorize('manager'), async (req, res) => {
  try {
    ...
    // Define the criteria list directly here
    const DETAILED_CRITERIA_LIST = [
      {
    ...
    let config = await JobReadinessConfig.findOne({ school, campus: null });
    ...
    // Append new criteria
    const newCriteriaObjects = DETAILED_CRITERIA_LIST.map(c => ({
      ...
      isActive: true,
      targetSchools: [school] // Explicitly target this school
    }));
    ...
    res.json({
      message: `Successfully added ${newCriteriaObjects.length} criteria to ${school}`,
      config
    });
