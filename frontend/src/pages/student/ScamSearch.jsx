...
import { utilsAPI, userAPI, scamReportsAPI, settingsAPI } from '../../services/api';
...
const normalizeResult = (raw = {}) => {
  ...
  return {
    ...
    verdict: raw.verdict || 'DANGER',
    summary: raw.summary || 'Expert scam analysis completed.',
    subScores: raw.subScores || { companyLegitimacy: 0, offerRealism: 0, processFlags: 0, communitySentiment: 0 },
    redFlags: Array.isArray(raw.redFlags) ? raw.redFlags : [],
    greenFlags: Array.isArray(raw.greenFlags) ? raw.greenFlags : [],
    communityFindings: (Array.isArray(raw.communityFindings) ? raw.communityFindings : []).map(f => ({
      ...f,
      sentiment: f.sentiment || (f.finding?.toLowerCase().includes('scam') ? 'negative' : 'neutral')
    })),
    salaryCheck: raw.salaryCheck || null,
    domainAnalysis: raw.domainAnalysis || null,
    resourceLinks: Array.isArray(raw.resourceLinks) ? raw.resourceLinks : [],
    ...
    searchMetadata: raw.searchMetadata || null
  };
};
...
const ScamDetector = () => {
  ...
  // Auto-scroll when result is loaded
  useEffect(() => {
  ...
  const handleAnalyze = async () => {
    try {
      ...
      // Debugging search results via Toast and Browser Console
      const meta = response.data?.searchMetadata;
      console.log('[ScamRadar] Search Metadata:', meta);

      if (meta) {
        if (meta.totalSources > 0) {
          const mode = meta.usedCse ? 'Google Custom Search' : 'Scraping Fallback';
          toast.success(`Found ${meta.totalSources} web sources via ${mode}`, { duration: 5000 });
        } else if (meta.queries > 0) {
          toast.error('Web search results were empty. Using Expert Heuristics.', { duration: 5000 });
        }
      }
  ...
  const handleTestSearch = async () => {
    if (!searchConfig.apiKey) {
      toast.error('Provide a Search API key (Google Cloud)');
      return;
    }
    try {
      setTestingSearch(true);
      setSearchTestResult(null);
      console.log('[TestSearch] Starting test for key:', searchConfig.apiKey.slice(0, 6) + '...');

      let cseId = '';
      try {
        const response = await settingsAPI.getSettings();
        const info = response.data?.data;
        cseId = (info?.customSearchEngineId || '').trim();
        console.log('[TestSearch] Global CSE ID found:', cseId ? cseId.slice(0, 8) + '...' : 'NOT SET');
      } catch (err) {
        console.warn('[TestSearch] Could not fetch global settings:', err.message);
      }

      if (!cseId) {
        console.warn('[TestSearch] Missing CX. Manager must configure Search Engine ID in Admin Settings.');
        toast.error('Manager has not configured a "Search Engine ID" in Admin Settings yet. Test aborted.');
        setSearchTestResult({ success: false, error: 'Missing CSE ID (cx) in platform settings' });
        return;
      }

      const testQuery = 'test';
      const url = `https://www.googleapis.com/customsearch/v1?key=${encodeURIComponent(searchConfig.apiKey)}&cx=${encodeURIComponent(cseId)}&q=${encodeURIComponent(testQuery)}`;
      ...
      if (data.error) {
        ...
        console.error('[TestSearch] Google API Error:', data.error);

        // Detailed Troubleshooting hints
        if (msg.toLowerCase().includes('custom search json api')) {
          msg = 'Permission Error: Enable "Custom Search JSON API" in the project where this key was created.';
        } else if (msg.toLowerCase().includes('access') || msg.toLowerCase().includes('forbidden')) {
          msg = 'Project Mismatch: This key might be from a different project than the one where you enabled the API.';
        }

        toast.error(msg, { duration: 8000 });
        setSearchTestResult({ success: false, error: msg });
      } else if (data.items) {
        console.log('[TestSearch] ✅ SUCCESS: Results found');
        toast.success(`Success! Found ${data.items.length} results via Google CSE.`);
        setSearchTestResult({ success: true, count: data.items.length });
      } else {
        console.warn('[TestSearch] ⚠️ No results found for query "test"');
        toast('API key is valid but search returned zero results. Check your "Sites to Search" in CSE dashboard.');
        setSearchTestResult({ success: true, count: 0 });
      }
    } catch (err) {
      const msg = err.message || 'Search API test failed';
      console.error('[TestSearch] Fatal error:', err);
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-24 sm:py-32 min-h-screen">
          ...
          <div className="space-y-6" ref={reportRef}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {result.salaryCheck && (
                <div className="bg-white rounded-3xl border border-gray-100 p-6 flex flex-col">
                  <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                    <DollarSign size={14} className="text-emerald-500" /> Salary Reality Check
                  </h3>
                  <div className="space-y-4 flex-1">
                    <div className="flex justify-between items-end border-b border-gray-50 pb-2">
                      <div>
                        <p className="text-[10px] font-black text-gray-300 uppercase">Offered</p>
                        <p className="text-lg font-black text-gray-900">{result.salaryCheck.offered}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-[10px] font-black text-gray-300 uppercase">Market Rate</p>
                        <p className="text-sm font-bold text-gray-600">{result.salaryCheck.marketRate}</p>
                      </div>
                    </div>
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className={`px-2 py-0.5 rounded-lg text-[10px] font-black uppercase ${result.salaryCheck.verdict === 'Realistic' ? 'bg-emerald-50 text-emerald-700' :
                          result.salaryCheck.verdict === 'Slightly High' ? 'bg-amber-50 text-amber-700' :
                            'bg-red-50 text-red-700'
                          }`}>
                          {result.salaryCheck.verdict}
                        </span>
                      </div>
                      <p className="text-sm text-gray-600 leading-relaxed italic">
                        {result.salaryCheck.explanation}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {result.domainAnalysis && (
                <div className="bg-white rounded-3xl border border-gray-100 p-6 flex flex-col">
                  <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                    <Globe size={14} className="text-blue-500" /> Domain & Email Forensics
                  </h3>
                  <div className="space-y-3 flex-1">
                    <div className="grid grid-cols-2 gap-2">
                      <div className="p-3 bg-gray-50 rounded-2xl border border-gray-100">
                        <p className="text-[9px] font-black text-gray-400 uppercase">Domain</p>
                        <p className="text-xs font-bold text-gray-900 truncate">{result.domainAnalysis.companyDomain || 'Unknown'}</p>
                      </div>
                      <div className="p-3 bg-gray-50 rounded-2xl border border-gray-100">
                        <p className="text-[9px] font-black text-gray-400 uppercase">Age</p>
                        <p className="text-xs font-bold text-gray-900">{result.domainAnalysis.domainAge || 'N/A'}</p>
                      </div>
                    </div>
                    <div className="p-3 bg-gray-50 rounded-2xl border border-gray-100 flex items-center justify-between">
                      <div>
                        <p className="text-[9px] font-black text-gray-400 uppercase">Risk Level</p>
                        <p className={`text-xs font-black uppercase ${result.domainAnalysis.domainRisk === 'Low' ? 'text-emerald-600' :
                          result.domainAnalysis.domainRisk === 'High' ? 'text-red-600' : 'text-amber-500'
                          }`}>
                          {result.domainAnalysis.domainRisk || 'Unknown'}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="text-[9px] font-black text-gray-400 uppercase">Sender Match</p>
                        <p className="text-[10px] font-bold text-gray-600">{result.domainAnalysis.senderDomainMatch || 'Checking...'}</p>
                      </div>
                    </div>
                    <p className="text-[11px] text-gray-500 italic px-1">{result.domainAnalysis.explanation}</p>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-white rounded-3xl border border-gray-100 p-6">
                <h3 className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-4 flex items-center gap-2">
                  <CheckCircle2 size={14} /> Safety Signals
                </h3>
                <div className="space-y-3">
                  {result.greenFlags.length > 0 ? result.greenFlags.map((f, i) => (
                    <div key={i} className="text-sm text-gray-700 flex items-start gap-2">
                      <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-emerald-400 shrink-0" />
                      <span>{f}</span>
                    </div>
                  )) : (
                    <p className="text-sm text-gray-400 italic">No significant safety signals identified.</p>
                  )}
                </div>
              </div>
            </div>

            {/* Restored Community Feedback Section */}
            <div className="bg-white rounded-3xl border border-gray-100 p-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <div>
                  <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
                    <Users size={14} /> Community Reports
                  </h3>
                  <p className="text-xs text-gray-400 mt-1 font-medium">Have you received this offer? Help others by voting.</p>
                </div>
                <div className="flex items-center gap-2 bg-gray-50 px-4 py-2 rounded-2xl border border-gray-100">
                  <div className="text-center px-3 border-r border-gray-200">
                    <p className="text-lg font-black text-gray-900 leading-none">{reportCount}</p>
                    <p className="text-[8px] font-black text-gray-400 uppercase">Total</p>
                  </div>
                  <div className="flex gap-4 pl-2">
                    <div className="text-center">
                      <p className="text-xs font-black text-red-500 leading-none">{currentReports.scam}</p>
                      <p className="text-[8px] font-black text-gray-300 uppercase">Scam</p>
                    </div>
                    <div className="text-center">
                      <p className="text-xs font-black text-emerald-500 leading-none">{currentReports.legit}</p>
                      <p className="text-[8px] font-black text-gray-300 uppercase">Legit</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <button
                  onClick={() => submitReport(result.company, 'scam')}
                  className="flex items-center justify-center gap-2 py-3 px-4 bg-red-50 hover:bg-red-100 text-red-700 rounded-2xl font-black text-[10px] uppercase tracking-wider transition-all border border-red-100"
                >
                  <AlertTriangle size={14} /> Confirmed Scam
                </button>
                <button
                  onClick={() => submitReport(result.company, 'legit')}
                  className="flex items-center justify-center gap-2 py-3 px-4 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 rounded-2xl font-black text-[10px] uppercase tracking-wider transition-all border border-emerald-100"
                >
                  <CheckCircle2 size={14} /> Legitimate
                </button>
                <button
                  onClick={() => submitReport(result.company, 'unsure')}
                  className="flex items-center justify-center gap-2 py-3 px-4 bg-gray-50 hover:bg-gray-100 text-gray-600 rounded-2xl font-black text-[10px] uppercase tracking-wider transition-all border border-gray-200"
                >
            ...
            {/* Restored Action Items & Detailed Verdict */}
            <div className="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm">
              <h3 className="text-[10px] font-black text-primary-600 uppercase tracking-widest mb-6 flex items-center gap-2">
                <Shield size={16} /> ScamRadar Verdict
              </h3>
              <p className="text-gray-700 leading-relaxed mb-8 font-medium italic border-l-4 border-primary-200 pl-4 py-1">
                {result.finalVerdict}
              </p>

              {result.actionItems && result.actionItems.length > 0 && (
                <div className="space-y-4">
                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Recommended Actions</p>
                  {result.actionItems.map((item, idx) => (
                    <div key={idx} className="flex items-center gap-4 p-4 bg-gray-50/50 rounded-2xl border border-gray-100 transition-all hover:bg-white hover:shadow-md group">
                      <div className="p-2 bg-white rounded-xl border border-gray-200 text-primary-600 font-black text-xs shadow-sm group-hover:bg-primary-600 group-hover:text-white group-hover:border-primary-600 transition-all">
                        {idx + 1}
                      </div>
            ...
            {result.communityFindings.length > 0 && (
              <div className="bg-white rounded-3xl border border-gray-100 p-8">
                <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-6 border-b border-gray-50 pb-4">
                  Community Intelligence
                </h3>
                <div className="space-y-6">
                  {result.communityFindings.map((f, i) => (
                    <div key={i} className="flex gap-5 group">
                      <div className="p-3 bg-gray-50 rounded-2xl border border-gray-100 text-gray-400 shrink-0 h-fit transition-all group-hover:bg-white group-hover:shadow-md">
                        {getIcon(f.icon, "w-5 h-5")}
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-1.5">
                          <p className="font-black text-xs text-gray-900 uppercase tracking-tight">{f.source}</p>
                          <span className={`px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest border ${f.sentiment === 'negative' ? 'bg-red-50 text-red-600 border-red-100' :
                            f.sentiment === 'positive' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' :
                              'bg-gray-50 text-gray-500 border-gray-200'
                            }`}>
                            {f.sentiment || 'Neutral'}
                          </span>
                        </div>
                        <p className="text-[13px] text-gray-600 leading-relaxed mb-3 font-medium">
                          {f.finding}
                        </p>
                        {Array.isArray(f.links) && f.links.length > 0 && (
                          <div className="flex flex-wrap gap-2">
                            {f.links.map((l, li) => (
                              <a key={li} href={l.link || l.url} target="_blank" rel="noopener noreferrer" className="text-[9px] font-black text-primary-600 hover:text-primary-700 bg-primary-50 px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-all border border-primary-100">
                                {l.title || 'View Source'} <ArrowUpRight size={10} />
                              </a>
                            ))}
                          </div>
                        )}
              ...
              <div className="flex flex-wrap items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                  <div className="flex -space-x-2">
                    {result.resourceLinks.slice(0, 3).map((r, i) => (
                      <div key={i} className="w-8 h-8 rounded-full bg-white border-2 border-white shadow-sm flex items-center justify-center text-primary-600">
                        {getIcon(r.icon, "w-4 h-4")}
                      </div>
                    ))}
                  </div>
                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Verify with external resources</p>
                </div>
                <div className="flex flex-wrap gap-3">
                  {result.resourceLinks.map((link, idx) => (
                    <a
                      key={idx}
                      href={link.url}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-[9px] font-black text-gray-500 hover:text-primary-600 flex items-center gap-1.5 transition-colors uppercase tracking-widest bg-gray-50 px-4 py-2 rounded-xl border border-gray-100 hover:border-primary-100 hover:bg-primary-50"
                    >
                      {getIcon(link.icon, "w-3 h-3")} {link.title}
                    </a>
                  ))}
                </div>
              </div>
      ...
      {showKeyModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[3rem] max-w-lg w-full max-h-[90vh] mx-auto shadow-2xl overflow-hidden flex flex-col">
