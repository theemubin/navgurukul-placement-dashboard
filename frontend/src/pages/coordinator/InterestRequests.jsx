...
function CoordinatorInterestRequests() {
    ...
    const fetchRequests = async () => {
        try {
            setLoading(true);
            // reusing the same API as it fetches all requests. 
            // Ensure backend allows coordinators to access this endpoint.
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-gray-600 mt-1">
                        View student requests to apply for jobs they don't fully qualify for
                    </p>
                </div>
                {pendingCount > 0 && (
            ...
            {filteredRequests.length === 0 ? (
                ...
                <div className="space-y-4">
                    {filteredRequests.map((request) => (
                        <Card key={request._id} className={`p-4 border-l-4 ${request.status === 'pending' ? 'border-l-yellow-500 bg-yellow-50' :
                    ...
                    <div className="space-y-6">
                        ...
                        {detailModal.request.status !== 'pending' && (
                            <div>
                                ...
                                <div className={`p-4 rounded-lg ${detailModal.request.status === 'approved' ? 'bg-green-50' : 'bg-red-50'
                                    }`}>
...
export default CoordinatorInterestRequests;
